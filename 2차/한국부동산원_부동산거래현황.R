...
trade_zone_year <- RegionAptTrade2 %>%
  mutate(
    권역 = ifelse(시도 %in% c('서울', '경기', '인천'), '수도권', '비수도권')
  ) %>%
  group_by(권역, 연도) %>%
  summarise(
    거래량 = sum(호수, na.rm=TRUE),
  ...
  arrange(권역, 연도)
head(trade_zone_year)

ggplot(trade_zone_year, aes(x=연도, y=거래량, color = 권역)) +
  geom_line() +
  geom_point() +
  labs(
    title = '수도권 vs 비수도권 거래 동향 비교',
    x= '연도',
    y= '거래량'
  ) +
  theme_minimal()
